...
|---|---|---|---|
| 海克斯 3 选 1（`2-1/3-2/4-2`，实测确认） | 3 张卡是谁 | 否（v1 用全局胜率即可） | 现成 |
| 神明祝福 2 选 1（`2-4/3-4/4-4`） | 2 张祝福卡 | 弱（1-1 介绍已锁定本局神明池） | 待调研 |
| 神之祝福 4 选 1（`4-7`） | 4 个装备/增益选项 | 弱 | 部分现成 |
| 商店买/D（每个 planning） | 5 槽 + 金币/等级 + **我方阵容** | **强**（board+bench） | 现成 |
| 装备合成 | 装备栏散件 | 弱 | 现成 |

**最短价值闭环 = 海克斯推荐**（检测已就绪、静止 2D 卡、卡上有中文名文本、资产可拉、外部数据现成、不依赖阵容），而不是商店。商店推荐需要「我有什么」，才是 board/bench 识别与 StateTracker 的真正客户——它们服务于商店那一刀，优先级据此排。
...
- **识别选项需资产**：海克斯 CDragon 有（Set 17 `augments` 276 项，详情在顶层 `items`，图标 `.tex`，可扩 `download_assets.py`）。**神明祝福卡和 1-1 介绍横幅都有清晰中文名文本**（「雷暴格子」「完整道具」等），识别可先走文字通道绕过图标资产问题；1-1 读出本局神明池后，`x-4` 的 2 选 1 在该先验内剪枝（同「费用查 catalog」思路）。
...
- **文字通道识别语料（已就位，2026-07-03）**：`augments_zh.json`（276 项海克斯中英文名 + 中文描述 + effects，含 17 个神明恩赐 GodAugment；源 = CDragon zh_cn；仅 2 个奥索任务型子恩赐缺描述）+ `gods_zh.json`（9 位神明名字/称号 + 128 个按 stage 分组的 offering；源 = Blitz `utils.iesdev.com/static/json/tftTest/set17/zh_cn/gods-v2`，需浏览器 UA）。均由 `download_assets.py` 生成。**识别决策定为文字优先**：整卡 OCR（名字+描述都是证据）→ CJK-only 归一化（顺带消掉 desc 的 `@Var@` 模板变量）→ 封闭集模糊匹配，margin 作 confidence；图标对比降级为可选交叉校验，augment 图标不再是前置（icon_url 已留在语料里备用）。同名 I/II/III 变体文字区分不了时靠卡面 tier 色带仲裁。另有 **`display_names_zh.json`（人工自举资产，非 download_assets 生成）**：国服屏幕显示名 ≠ CDragon 官方名的覆盖映射（Set 17 主题化改名面很大），识别时作为名字通道附加变体；配套的台服→国服词汇归一表在 `tools/ocr_probe.py` 的 `_CANON`。
...
4. 🟨 **海克斯选项识别 + 推荐闭环**：✅ OCR 离线实验（`tools/ocr_probe.py`，15 帧）——RapidOCR 在 1280 降采样帧上**标题全对**、描述基本完整（1.7s/帧 CPU），文字通道可行性确认。**但发现语料瓶颈**：客户端 Set 17 对部分海克斯做了主题化改名+数值修订（屏幕「腰带过载/尺有所短、寸有所长」vs 一切公开表「腰带溢流/尺寸至上」——CDragon 与腾讯 `game.gtimg.cn/.../hex.js` 是同一套旧译名）。✅ **匹配打分 v2 → 12/12 全对**（基线 ~4/12）：max(名字,描述) 双通道（合并 bigram 会互相稀释——珠光莲花名字 1.00 曾输给合并 0.44 的错误候选）+ 平滑 containment `hits/(|cand|+3)` + 最小交集门槛 3 + 垃圾过滤（name_zh 含 `@` 的 4 条：GainGold/奥索任务恩赐）+ **台服→国服词汇归一**（CDragon zh_cn 实为台服系用词：弈子/机率/潘朵拉/备战席 vs 国服屏幕 英雄/几率/潘多拉/备战区，两侧过 `_CANON` 表归一后描述通道才可比）。✅ **国服显示名覆盖表** `assets/set17/display_names_zh.json`（首批 10 条自举：desc 锚定身份 → 人工核对 → 记录屏幕标题；probe 对「描述胜出但名字 0 分」的卡打 ⚠ 提示新改名，可逐局滚大）——**改名面远超预想：12 卡中 10 卡国服名 ≠ CDragon**（刃马合一=征战之路、思绪杂乱=纷乱头脑、战神的荣耀=猛将的荣耀、重整装置=变形重组器、混乱的呼唤=混沌召唤、黄金豪赌=黄金赌约…）。✅ 重掷在实录中实锤（3-2 重掷 1 次、4-2 两次，且是**单卡独立重掷**而非整组刷新），probe 一致性检查已改为重掷分段统计。残留：I/II/III 同族变体 margin 仅 0.01–0.05、单帧 OCR 罗马数字会翻（珠光莲花 I↔II）→ 按既定方案 tier 色带仲裁 + 多帧投票，不在文字通道内解决。✅ **识别 → 评级 → UI 闭环已接（2026-07-04）**：`CardRecognizer`（`tft_mvp/recognize/cards.py`，复用 probe v2 配方 + exact 捷径 + 自举覆盖表 `name_overrides.json` 自动学习）挂上 Pipeline——决策窗口内节流 OCR（未锁 1.2s / 锁定后 3s 复核），2 帧一致锁定，选项组签名变化（重掷）自动重投票；state 出 `decision` 块（api + 国服显示名 + MetaTFT tier + **中文 tag**：经济/战斗/装备/羁绊/成长/功能，译表 `reco.TAGS_ZH`）；Dashboard 加 DECISION 区（5 色 tier 徽章 + 中文走 PIL 叠加渲染——cv2 画不了 CJK，字体候选 Windows 微软雅黑 / Linux Noto，缺字体降级 ASCII）；**track 记 `decision` 事件**（锁定时一条、重掷出新组再一条，`--record` 的 track.json 因此含整局海克斯/神明识别+评级记录）。全链路已用实录帧回放验证（4-2 三卡 B/B/A 正确锁定 + track 事件落盘 + 面板渲染核对）。environment.yml 已加 `rapidocr-onnxruntime`（pip 段）。⬜ 剩余：Windows 全分辨率 live 实测；卡 ROI 改由 PhaseMarker 面板边界供给（现为固定 band）；推荐话术（tier 已在，「选哪张」一句话输出）
5. 🟨 神明祝福 2 选 1（`x-4`）：初版已通——`match_god` 两步匹配（称号锚定神明 → 该神 offering 内配祝福；评级用该神恩赐海克斯 `*GodAugment*` 做代理）+ **零证据门槛**（god_confidence<0.3 整次读取作废，防走动段误聚列被投票锁定，实测已拦）。实录帧验证：**x-4 两张卡来自本局两位神明各一张**（亚索·深渊之神「风暴格」vs 伊芙琳·诱惑之神「12金币」，均 conf 1.00）。⬜ 剩余：GOD_BAND ROI 未正式标定（现值在浮层帧 2/2 命中）；offering 的 `TFT17_Benefit_*`（金币/经验类）无公开评级，UI 暂只显神明代理 tier；神之祝福 4 选 1（`4-7`）未做
...
**两个实操注意**：`frames_out` 是 1280 宽降采样帧，够标 ROI，但识别决策卡要吃 live 管线的全分辨率帧；国服客户端是中文名，外部胜率库多为英文键，靠 `manifest.json` 的中英文映射 + `apiName` 对齐。
...
- **`--record track.json`**：整局时间轴 track（数值，小）。逐帧样本含 `stage/round/countdown/sr_status` + `scene/n_panels/bluepurple`（marker 结果一并落盘，供离线校准阈值）；退出时落盘、每 40 帧增量存防崩溃。
...
- **回环**：Windows 采集 → 发回 `track.json` + `frames_out/` → Linux 用帧标定 ROI（备战席 / 棋盘）、裁实机模板、多帧校准 marker。首局完整实录已经用这条回环确认了：海克斯回合 = `2-1/3-2/4-2`、`x-4` 是神明祝福回合（Set 17 无选秀）、`4-7` 是神之祝福、god 检测语义要重构。这就是「平台相关采集 / 平台无关分析」分离的落地闭环。
