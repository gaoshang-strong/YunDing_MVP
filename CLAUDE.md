...
> **为什么放弃纯颜色占比**：最初用「中带紫色占比 ≥0.08」判海克斯，单帧分离 50×，但**多帧真实对局翻车**——它把**选秀转盘**（紫色魔法 UI）和偶发紫色特效一起误判（散在十几个回合）。教训：单帧标定的颜色阈值在全局数据上分离度会塌。

| 阶段 | 信号 | 判据 |
|---|---|---|
| `augment_select` | 中带**面板结构**（`markers.augment_band` 列投影） | 恰 **3 个等宽面板**（宽 0.18–0.35、max/min<1.6）。海克斯是 3 张并排卡片（实测 0.26/0.26/0.26）；选秀转盘 / 普通阶段不成 3 等宽面板 → 天然排斥 |
| `god_select` | 全屏**深蓝紫**占比（`markers.overlay_full`） | ≥ 0.40 **且 stage≤1**。神明实测 0.52–0.70、普通 0.02–0.05，26 帧零误报；神明立绘不规则、结构不干净，故用颜色 |
...
- **固定回合先验**：海克斯回合固定（传统 `2-1/3-2/4-2`），`at_augment_round` 作软校验输出，**不硬 gate**（怕补丁改动）；Set 17 确切回合待数据确认。神明 `1-1` 已用 stage≤1 兜底。
- **浮层不遮顶栏，但海克斯会把顶栏调暗**（见 ClockTrack 坑）；神明期间顶栏正常。
- **已验证**：海克斯 2 帧（`s_00012`+录像 `v_009`）、神明 `s_00001`、~40 普通帧全对。**选秀拒绝待下次录制实证**（本地录像仅 ~4.3min，无选秀帧）。
- **识别选项**（3 海克斯 / 2 神明是谁）需资产：海克斯 CDragon 有（Set 17 `augments` 276 项，详情在顶层 `items`，图标 `.tex`，可扩 `download_assets.py`）；**神明数据源未确认**（CDragon tft json 里搜不到"神明/女神/神祇"，待调研）。
