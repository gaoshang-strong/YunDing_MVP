...
- 挂在 `Pipeline`，输出进 state 的 **`track` 块**（最近样本 + 全部事件 + `cd_trend` 倒计时趋势）。`tools/live.py --record track.json` 可把整局 track 落盘。
...
**关键区分：「检测界面」≠「识别选项」，前者不需要内容资产**。判「当前在不在海克斯/神明界面」只要一个结构 marker，**不需要**海克斯/神明图标资产（那些确实还没下载，见待定）；识别「是哪 3 个海克斯」才需要资产。所以**阶段检测不被缺资产卡住，现在就能做**。已从 survey 真实帧确认的 marker 线索（`s_00012`=海克斯、`s_00001`=选神明/开局）：
- **augment_select**：中间横带 **3 块紫色亮面板** + 顶部「选一个」字样。棋盘底色是绿/蓝，紫色在中心区是极强信号 → HSV 紫色掩膜在固定中心 ROI 算占比即可，可选加字样模板校验。**浮层不遮顶栏，但会把顶栏调暗**（见 ClockTrack 实测坑）。
- **god_select**：仅 `1-1` 出现一次（倒计时长，`s_00001` 为 240），左右两大神明立绘 + 特殊魔法背景 → 可用 `stage-round==1-1` + 背景特征兜底，低优先。
- **carousel / pve_loot**：survey 未抓到干净帧，需从录像补。
...
2. ✅ ClockTrack：时间轴 track（逐帧样本 + 派生事件，`--record` 导出 JSON）——阶段判定的原料
3. ⬜ 阶段判定（planning / combat / unknown 四层）——先用真实对局 track 数据观察规律再定阈值
4. ⬜ 商店五槽
