...
class ClockTrack:
    ...
    def __init__(self, recent: int = 10, recent_rounds: int = 2):
        self.recent = recent
        self.recent_rounds = recent_rounds
        self.rounds = RoundTrack()  # 「回合 → 4 小阶段」结构化
        self.reset()
    ...
    def snapshot(self) -> dict:
        """给 state dict 的精简 track 块（最近样本 + 全部事件 + 趋势 + 最近 N 回合小阶段）。"""
        return {
            ...
            "events": self.events,
            "rounds": self.rounds.snapshot(self.recent_rounds),
        }
